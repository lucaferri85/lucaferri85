...
## Regression analysis vs last known-good detector (Jun 2026)
- Last known-good (v0, previous agent, uncommitted → overwritten, not recoverable from git/babel cache) produced 26/30 on the real warrior mainly because it filled structural joints from **proportional priors** (LOW/MEDIUM) when geometry evidence failed. v1 (590c347) removed priors + added plausibility gates with single points of failure (crotch → 12 landmarks; armpit/torsoHalf → 10) and x-only clustering → 3/30 on the caped warrior. v2 (cf0a1e4) fixed cascades but still lost elbows/wrists/hands (6) on a mannequin with arms hanging close to the torso (armpit separation found far below the real armpit → arm length "implausible").
- Controlled fix (v3): (a) `meshSampler` density-invariant (no vertex-per-triangle at high triangle counts; ≈80 k points for 100 or 3 M triangles; streaming sampler in viewport — 9 M-vertex meshes no longer inflate the cloud); (b) grid cell 0.8 %H (separates arms/legs with ≥ ~2.5 cm gaps); (c) `central()` ignores debris components (< 15 % of the largest) — helmet spike no longer becomes the "neck"; (d) close-arms branch: when the arm separation is > 14 %H below the neck base, recover the hanging-arm axis from the separated lower arm (least squares) and extrapolate to shoulder height (MEDIUM, verify note); elbow on the axis when the upper arm is merged; (e) arm-length lower bound 25 %H. Results (node + browser identical): mannequin 26/30 (H5 M21 NF4), humanoid 26/30 (H7 M19), warrior 26/30 (H6 M20), cape 26/30 (H4 M18 L4), tabard 26/30, cape+tabard 26/30, T-pose 26/30; ×64 subdivided meshes give the same landmarks within ~2 cm.
